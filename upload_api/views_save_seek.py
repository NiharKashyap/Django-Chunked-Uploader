...
def upload_partial(request):
    if request.method=="GET":
        ...
        file_path = os.path.join(settings.MEDIA_ROOT, 'files\\' , request.POST["file_id"] + "_" + request.POST["file_name"])
        f = open(file_path, "wb")
        print(request.POST["start"])
        print('chunk ', request.POST["chunk"])
        f.seek(int(request.POST["start"]))
